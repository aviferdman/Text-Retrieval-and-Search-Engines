...
#             hits_qld = searcher_qld.search(query[1], k=1000)
#             hits_bm25 = searcher_bm25.search(query[1], k=1000)
#
...
#             for rank, (doc_id, score) in enumerate(normalized_hits, start=1):
#                 f.write(f"{query_id} Q0 {doc_id:<17} {rank:<4} {score:<20.6f} run_2\n")
#
#     # Read results and prepare DataFrame for reranking
#     model = train_reranker([('run_2.res', 'run_2')])
#
#     # Rerank using the trained LambdaMART model
#     df = read_scores_from_files([('run_2.res', 'run_2')])
